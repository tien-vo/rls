...
class SimulationTrack(Simulation):
    def run(
        self,
        initial_conditions: Particle,
        run_time: float = 1.0,
        ...
        for n in range(run_steps):
            ...
            # ---- Drift: Rotate velocity with the fields at half-step
            _Iw, Ex, Ey, Ez, Bx, By, Bz = field(t, x, Iw, z, *field_args)
            g, ux, uy, uz = push_u(
                ux, uy, uz, Ex, Ey, Ez, Bx, By, Bz, *push_args
            )
            ...
            Iw += _Iw * np.abs(z - z_old)

            # Wrap particles
            if pbc:
                x = (x + L_wrap / 2) % L_wrap - L_wrap / 2
                z = (z + L_wrap / 2) % L_wrap - L_wrap / 2
