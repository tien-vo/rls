__all__ = ["WhistlerAtHyperbolicGradientModel"]
...
class WhistlerAtHyperbolicGradientModel(HyperbolicGradientModel):
    ...
    @nb.njit(
        [
            f"UniTuple(f8[::1], 6)(f8, {'f8[::1],' * 3} {'f8,' * 13})",
            f"UniTuple(f8[:, ::1], 6)(f8, {'f8[:, ::1],' * 3} {'f8,' * 13})",
            f"UniTuple(f8[::1], 6)({'f8[::1],' * 4} {'f8,' * 13})",
            f"UniTuple(f8[:, ::1], 6)({'f8[:, ::1],' * 4} {'f8,' * 13})",
        ],
        cache=True,
    )
    def field(
        t, x, y, z, Bw, w_wce, theta, xw, zw, sw, R, Bh, B0, wpe0, qe, me, c
    ):
        args_1 = (R, Bh, B0)
        args_2 = (Bw, w_wce, theta, xw, zw, sw, wpe0, qe, me, c)
        _, _, _, B0_x, _, B0_z = background_field(t, x, y, z, *args_1)
        Ew_x, Ew_y, Ew_z, Bw_x, Bw_y, Bw_z = wave_field(
            t, x, y, z, B0_x, B0_z, *args_2
        )
