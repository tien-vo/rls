...
eta = jitter(lambda z, R, Bh, B0: 1 - (Bh / B0) * np.exp(-(z**2) / 2 / R**2))
d_eta = jitter(
    lambda z, R, Bh, B0: z / R**2 * (Bh / B0) * np.exp(-(z**2) / 2 / R**2)
)


class MagneticHoleModel(UniformPlasmaModel):
